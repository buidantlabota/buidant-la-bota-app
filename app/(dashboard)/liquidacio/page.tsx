...
function DebouncedInput({
  ...
  return (
    <input
      ...
      onBlur={() => {
        const finalValue =
          type === "number" ? parseFloat(localValue) || 0 : localValue;
...
export default function LiquidacioPage() {
  ...
  const saveSummary = (ef: number, tr: number, ba: number) => {
    ...
    localStorage.setItem(
      key,
      JSON.stringify({ efectiu: ef, transfer: tr, banc: ba }),
    );
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (selectedYears.length < AVAILABLE_YEARS.length) {
        const yearFilters = selectedYears
          .map((y) => `and(data_bolo.gte.${y}-01-01,data_bolo.lte.${y}-12-31)`)
          .join(",");
  ...
  const handleUpdateAjustament = async (
    boloId: number,
    musicId: string,
    value: number,
  ) => {
    try {
      ...
      setAttendance((prev) =>
        prev.map((a) =>
          a.bolo_id === boloId && a.music_id === musicId
            ? { ...a, ajustament_preu: value }
            : a,
        ),
      );
    } catch (error) {
  ...
  const handleUpdateComentari = async (
    boloId: number,
    musicId: string,
    text: string,
  ) => {
  ...
  const handleUpdateGlobal = async (
    musicId: string,
    field: "sobre_fet" | "ajustament_global",
    value: number | boolean,
  ) => {
    const targetYear = Math.max(...selectedYears.map((y) => parseInt(y)));

    try {
      const current = globals.find(
        (g) => g.music_id === musicId && g.exercici === targetYear,
      );
      ...
      if (current) {
        ...
        const { error } = await supabase
          .from("liquidacio_global")
          .insert([
            { exercici: targetYear, music_id: musicId, [field]: value },
          ]);
        if (error) throw error;
      }
      ...
      setGlobals((prev) => {
        ...
        if (exists) {
          return prev.map((g) =>
            g.music_id === musicId && g.exercici === targetYear
              ? { ...g, [field]: value }
              : g,
          );
        } else {
          return [
            ...prev,
            {
              ...
              ajustament_global:
                field === "ajustament_global" ? (value as number) : 0,
  ...
  const getInstrumentPriority = (inst: string): number => {
    const i = inst
      ...
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "");
    if (i.includes("percussio") || i.includes("caixa") || i.includes("bombo"))
      return 1;
    ...
    if (i.includes("saxo") && (i.includes("tenor") || i.includes("terror")))
      return 6;
  ...
  const groupedMusicians = useMemo(() => {
    const titulars = musics
      .filter((m) => m.tipus === "titular")
      .sort((a, b) => {
        const prioA = getInstrumentPriority(
          a.instrument_principal || a.instruments || "",
        );
        const prioB = getInstrumentPriority(
          b.instrument_principal || b.instruments || "",
        );
        if (prioA !== prioB) return prioA - prioB;
        return a.nom.localeCompare(b.nom);
      });

    const substituts = musics
      .filter((m) => m.tipus === "substitut")
      .sort((a, b) => {
        const prioA = getInstrumentPriority(
          a.instrument_principal || a.instruments || "",
        );
    ...
    const othersIds = new Set(
      attendance.map((a) => a.music_id).concat(globals.map((g) => g.music_id)),
    );
    const taggedIds = new Set([...titulars, ...substituts].map((m) => m.id));
    const others = musics.filter(
      (m) => othersIds.has(m.id) && !taggedIds.has(m.id),
    );
  ...
  const grandTotal = useMemo(() => {
    ...
    groupedMusicians.forEach((group) => {
      group.list.forEach((music) => {
        ...
        bolos.forEach((bolo) => {
          if (selectedBolos[bolo.id]) {
            const att = attendance.find(
              (a) => a.bolo_id === bolo.id && a.music_id === music.id,
            );
            if (att && att.estat === "confirmat") {
              musicBase +=
                att.preu_personalitzat !== null
                  ? att.preu_personalitzat
        ...
        const musicGlobalAjust = musicGlobals.reduce(
          (acc, curr) => acc + (curr.ajustament_global || 0),
          0,
        );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 px-2 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium">
            Control de pagaments als músics
          </p>
        </div>
        ...
        <table className="text-left border-collapse text-[10px] w-full min-w-[max-content]">
          <thead>
            ...
            <tr className="bg-gray-100 border-b border-gray-200">
              <th className="p-2 border-r border-gray-200 sticky left-0 bg-gray-100 z-10 w-48">
                PAGAR? (Selector)
              </th>
              <th className="p-2 border-r border-gray-200 text-center font-black w-24">
                BOLOS
              </th>
              <th className="p-2 border-r border-gray-200 text-center font-black w-24">
                AJUSTAMENTS
              </th>
              <th className="p-2 border-r border-gray-200 text-center font-black w-24 text-red-600">
                SOBRE FET
              </th>
              <th className="p-2 border-r border-gray-200 text-center font-black w-24 bg-primary/10 text-primary">
                TOTAL NET
              </th>
              {bolos.map((bolo) => (
                <th
                  key={`sel-${bolo.id}`}
                  className="p-1 border-r border-gray-200 text-center w-24"
                >
                  <div className="flex flex-col items-center">
                    ...
                    />
                    <span
                      className={`mt-1 font-bold ${selectedBolos[bolo.id] ? "text-green-600" : "text-gray-400"}`}
                    >
            ...
            <tr className="bg-gray-50 border-b border-gray-200 font-black">
              <th className="p-2 border-r border-gray-200 sticky left-0 bg-gray-50 z-10">
                MÚSICS
              </th>
              <th className="p-2 border-r border-gray-200 text-center">Base</th>
              <th className="p-2 border-r border-gray-200 text-center text-primary">
                (+/-)
              </th>
              <th className="p-2 border-r border-gray-200 text-center text-red-600">
                Ja lliurat
              </th>
              <th className="p-2 border-r border-gray-200 text-center bg-primary/10 text-primary">
                LIQUIDACIÓ
              </th>
              {bolos.map((bolo) => (
                <th
                  key={`name-${bolo.id}`}
                  className="p-2 border-r border-gray-200 text-center align-top min-w-[80px]"
                >
            ...
            {groupedMusicians.map((group) => (
              <Fragment key={group.title}>
                <tr className="bg-gray-900 text-white font-black text-[9px] uppercase tracking-widest">
                  <td colSpan={bolos.length + 5} className="px-3 py-1">
                    {group.title}
                  </td>
                </tr>
                {group.list.map((music) => {
                  ...
                  const musicGlobals = globals.filter(
                    (g) => g.music_id === music.id,
                  );
                  const latestYear = Math.max(
                    ...selectedYears.map((y) => parseInt(y)),
                  );
                  const latestGlobal = musicGlobals.find(
                    (g) => g.exercici === latestYear,
                  );

                  const sumaAjustGlobal = musicGlobals.reduce(
                    (acc, curr) => acc + (curr.ajustament_global || 0),
                    0,
                  );
                  ...
                  const totalNet =
                    sumaBaseBolos + sumaAjustaments + sumaAjustGlobal;
                    ...
                    >
                      <td className="p-2 border-r border-gray-200 sticky left-0 bg-white z-10 font-bold whitespace-nowrap group-hover:bg-gray-50">
                        <div className="flex flex-col">
                          <span>{music.nom}</span>
                          <span className="text-[7px] font-medium opacity-50 uppercase">
                      ...
                      <td className="p-2 border-r border-gray-200 text-center font-bold text-primary group/ajust relative">
                        <div className="flex flex-col items-center">
                          <PrivacyMask
                            value={sumaAjustaments + sumaAjustGlobal}
                          />
                          <DebouncedInput
                            type="number"
                      ...
                      <td className="p-2 border-r border-gray-200 text-center font-bold text-red-600">
                        <div className="flex flex-col items-center">
                          ...
                          />
                          <span className="text-[7px] mt-1 uppercase opacity-50">
                            {sobreFet ? "Fet" : "Pendent"}
                          </span>
                        </div>
                      </td>
                      {/* Green background only when sobreFet is true. Otherwise use a light warning color if > 0. */}
                      <td
                        className={`p-2 border-r border-gray-200 text-center font-black text-xs transition-colors duration-300 ${sobreFet ? "bg-green-600 text-white" : totalNet > 0 ? "bg-orange-50 text-orange-600" : "bg-gray-50 text-gray-400"}`}
                      >
                      ...
                      {bolos.map((bolo) => {
                        const att = attendance.find(
                          (a) =>
                            a.bolo_id === bolo.id && a.music_id === music.id,
                        );
                              ...
                              <div className="flex flex-col items-center">
                                <span className="font-black text-gray-900">
                                  {(att!.preu_personalitzat !== null
                                    ? att!.preu_personalitzat
                                    : bolo.preu_per_musica || 0) +
                                    (att!.ajustament_preu || 0)}
                                  €
                                </span>
                                {(att!.ajustament_preu !== 0 ||
                                  att!.comentari_ajustament) && (
                                  <div className="group/info relative mt-1">
                                    <span className="material-icons-outlined text-[10px] text-primary cursor-help">
                                      info
                                    </span>
                                    <div className="hidden group-hover/info:block absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-gray-900 text-white text-[8px] p-2 rounded shadow-lg pointer-events-none">
                                      <p className="font-bold">
                                        Ajustament: {att!.ajustament_preu || 0}€
                                      </p>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                ...
                <DebouncedInput
                  ...
                  onSave={(v: number) => {
                    setSummaryEfectiu(v);
                    saveSummary(v, summaryTransfer, summaryBanc);
                  }}
                />
              </div>
              <div className="flex flex-col gap-1">
                ...
                <DebouncedInput
                  ...
                  onSave={(v: number) => {
                    setSummaryBanc(v);
                    saveSummary(summaryEfectiu, summaryTransfer, v);
                  }}
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <DebouncedInput
                ...
                onSave={(v: number) => {
                  setSummaryTransfer(v);
                  saveSummary(summaryEfectiu, v, summaryBanc);
                }}
              />
            </div>
            ...
            <div className="flex justify-between items-center px-1">
              ...
              <span
                className={`text-sm font-black ${grandTotal - (summaryEfectiu + summaryBanc + summaryTransfer) === 0 ? "text-green-600" : "text-orange-600"}`}
              >
                <PrivacyMask
                  value={
                    grandTotal -
                    (summaryEfectiu + summaryBanc + summaryTransfer)
                  }
                  showEuro={true}
                />
              </span>
