...
export default function EconomiaPage() {
  ...
  const fetchEconomia = async (silent = false) => {
    ...
    if (error) {
      ...
      const ing = (data || []).reduce(
        (acc: number, b: any) => acc + (b.import_total || 0),
        0,
      );
      const cost = (data || []).reduce(
        (acc: number, b: any) => acc + (b.cost_total_musics || 0),
        0,
      );
      const p = (data || []).reduce(
        (acc: number, b: any) => acc + (b.pot_delta_final || 0),
        0,
      );

      const ingEfectiu = (data || [])
        .filter((b: any) => b.tipus_ingres === "Efectiu")
        .reduce((acc: number, b: any) => acc + (b.import_total || 0), 0);
      const ingFactura = (data || [])
        .filter((b: any) => b.tipus_ingres === "Factura")
        .reduce((acc: number, b: any) => acc + (b.import_total || 0), 0);
      ...
      const { data: allBolos } = await supabase
        .from("bolos")
        .select(
          "import_total, cost_total_musics, ajust_pot_manual, pot_delta_final, data_bolo, cobrat, pagaments_musics_fets, estat",
        )
      ...
      const { data: allAdvances } = await supabase
        ...
        .select(
          "import, data_pagament, bolos(estat, data_bolo, cobrat, pagaments_musics_fets)",
        )
        .gte("data_pagament", cutoffDate);

      const manualBalance = (allMovements || []).reduce(
        (sum: number, m: any) =>
          sum + (m.tipus === "ingrés" ? m.import : -m.import),
        0,
      );
      ...
      const potRealCount = (allBolos || [])
        .filter(
          (b: any) =>
            b.data_bolo >= cutoffDate && b.cobrat && b.pagaments_musics_fets,
        )
        .reduce((sum: number, b: any) => sum + (b.pot_delta_final || 0), 0);

      let dinersDispoImpact = 0;
      (allBolos || [])
        .filter((b: any) => b.data_bolo >= cutoffDate)
        .forEach((b: any) => {
          if (b.cobrat) {
            dinersDispoImpact +=
              (b.import_total || 0) + (b.ajust_pot_manual || 0);
          }
      ...
      const finalPotReal =
        potBase + manualBalance + potRealCount - pendingAdvancesForPotReal;
      const finalDinersDispo =
        potBase + manualBalance + dinersDispoImpact - pendingAdvancesForDispo;
  ...
  const handleUpdate = async (id: number, field: string, value: any) => {
    ...
    setBolos((prev) =>
      prev.map((b) => (b.bolo_id === id ? { ...b, [field]: value } : b)),
    );
    ...
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          {/* Year Selector */}
          <div
            className={`flex items-center space-x-2 transition-opacity ${showCurrentMonth ? "opacity-50 pointer-events-none" : "opacity-100"}`}
          >
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm font-mono font-bold text-red-400">
                    -{stats.aPagar.toFixed(0)}€
                  </div>
                </div>
              </div>
            </div>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-tight mt-4 relative z-10">
              Consolidat al banc + despeses manuals
            </p>
          </div>
          ...
          <div className="bg-emerald-950 text-white p-8 rounded-3xl shadow-xl border border-white/10 relative overflow-hidden flex flex-col justify-between min-h-[160px] group hover:scale-[1.01] transition-transform duration-300">
            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-sm font-bold text-emerald-400/80 mb-1">
                    Diners a disposició
                  </h3>
                ...
                <div className="text-right">
                  <div className="text-[10px] font-bold text-emerald-400/60 uppercase">
                    Pendent de cobrar
                  </div>
                  <div className="text-sm font-mono font-bold text-emerald-400">
                    +{stats.aCobrar.toFixed(0)}€
                  </div>
                </div>
              </div>
            </div>
            <p className="text-emerald-600 text-[10px] font-bold uppercase tracking-tight mt-4 relative z-10">
              Marges de bolos cobrats (Pendents de pagar)
            </p>

            <div className="mt-4 pt-4 border-t border-white/10 space-y-2 relative z-10">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black uppercase text-emerald-400/50">
                  Distribució Real
                </span>
                <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                  Ingressos {selectedYear}
                </span>
                <span className="material-icons-outlined text-emerald-500 text-lg group-hover:scale-110 transition-transform">
                  trending_up
                </span>
              </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between group hover:shadow-md transition-shadow duration-300">
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                  Cost Músics {selectedYear}
                </span>
                <span className="material-icons-outlined text-red-500 text-lg group-hover:scale-110 transition-transform">
                  group
                </span>
              </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-50">
              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter italic">
                Suma de pagaments previstos i fets
              </p>
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-[10px] font-black uppercase text-primary tracking-widest">
                  Rendiment {selectedYear}
                </span>
                <span className="material-icons-outlined text-primary text-lg animate-pulse-slow">
                  account_balance_wallet
                </span>
              </div>
              <h3 className="text-xs font-bold text-slate-500 mb-1">
                Marge directe dels bolos
              </h3>
              <p
                className={`text-3xl font-black font-mono tracking-tight ${stats.pot >= 0 ? "text-primary" : "text-red-500"}`}
              >
                {stats.pot.toFixed(2)}€
              </p>
            </div>
            <div className="mt-4 pt-4 border-t border-primary/10">
              <p className="text-[9px] font-bold text-primary/60 uppercase tracking-tighter italic">
                Som la polla (Marge net anual)
              </p>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
              ...
              {loading && bolos.length === 0 ? (
                ...
                bolos.map((bolo) => (
                  <Fragment key={bolo.bolo_id}>
                    <tr
                      className={`hover:bg-gray-100 transition-colors group ${expandedBolo === bolo.bolo_id ? "bg-gray-100" : ""}`}
                    >
                      ...
                      <tr className="bg-white">
                        <td colSpan={9} className="p-4 pl-12">
                          <div className="bg-white rounded border border-border p-4 shadow-inner">
                            <h4 className="text-sm font-bold mb-2 text-text-primary">
                              Músics Assignats
                            </h4>
                            {loadingMusics ? (
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                {boloMusics.map((bm: any) => (
                                  <div
                                    key={bm.id}
                                    className="text-xs flex justify-between items-center p-2 rounded bg-background border border-border"
                                  >
                                    <div>
            ...
            <tfoot className="bg-white border-t-2 border-border font-bold text-text-primary">
              <tr>
                ...
                <td
                  className={`p-4 text-right font-mono ${stats.pot >= 0 ? "text-primary" : "text-red-500"}`}
                >
                  {stats.pot.toFixed(2)}€
                </td>
                <td className="p-4"></td>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl scale-in-center">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-black tracking-tight text-slate-800">
                Distribució de Diners a Disposició
              </h2>
              <button
            ...
            <p className="text-xs text-gray-400 mb-6 font-medium italic">
              Distribueix mentalment els calers que tens ingressats (i encara no
              pagats) per saber què hi ha a cada compte bancari o associació.
            </p>
            ...
            <div className="space-y-3">
              {distribution.map((item, index) => (
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setDistribution(
                        distribution.filter((_, i) => i !== index),
                      )
                    }
              ...
              <div className="flex gap-2 p-2 rounded-xl border-2 border-dashed border-gray-100 italic text-gray-400 hover:border-gray-200 transition-colors">
                <input
                  ...
                  onChange={(e) =>
                    setNewDistItem({ ...newDistItem, name: e.target.value })
                  }
                />
                <input
                  type="number"
            ...
            <div className="flex gap-4 mt-8">
              ...
              <button
                onClick={() => handleSaveDist(distribution)}
                className="flex-1 py-3 bg-slate-900 text-white rounded-xl font-bold uppercase text-xs shadow-xl shadow-gray-200"
              >
                Guardar
              </button>
            </div>
