...
import {
    MonthlyEvolutionChart, VolumeVsRevenueChart, TopTownsChart,
    PaymentDistributionChart, PriceRangesChart
...
const MapWidget = dynamic(() => import('@/components/estadistiques/MapWidget'), {
    ssr: false,
    loading: () => (
        <div className="h-[500px] w-full bg-gray-100 animate-pulse rounded-[2.5rem] flex items-center justify-center text-gray-400 font-bold uppercase tracking-widest">
            Carregant Mapa...
        </div>
    ),
});

const ComparePanel = dynamic(() => import('@/components/estadistiques/ComparePanel'), {
    ssr: false,
    loading: () => (
        <div className="h-64 w-full bg-gray-100 animate-pulse rounded-[2.5rem] flex items-center justify-center text-gray-400 font-bold uppercase tracking-widest">
            Carregant Mode Compara...
        </div>
    ),
});
...
// ── Memoised big chart sections to prevent costly re-renders ──
const ChartSection = memo(function ChartSection({ stats }: { stats: any }) {
    return (
        <>
            {/* Financial & Evolution */}
            ...
            <section className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-1 bg-white p-8 rounded-[2.5rem] border border-gray-100 flex flex-col justify-center items-center text-center">
                    ...
                    <h3 className="text-4xl font-black text-gray-900 mb-2">
                        {Math.round(stats?.kpis?.acceptanceRate || 0)}%
                    </h3>
...
export default function EstadistiquesPage() {
    ...
    const [metaData, setMetaData] = useState({ years: [] as string[], towns: [] as string[], types: [] as string[] });
    const [stats, setStats] = useState<any>(null);
    const [filters, setFilters] = useState<any>(null);
    const [mode, setMode] = useState<Mode>('normal');

    // ── 1. Load filter metadata once ─────────────────────
    useEffect(() => {
        const fetchMeta = async () => {
            const { data } = await supabase
                .from('bolos')
                .select('data_bolo, nom_poble, tipus_actuacio');
            if (data) {
    ...
    // ── 2. Fetch stats (debounced via FilterPanel) ────────
    const fetchStats = useCallback(async (currentFilters: any) => {
        ...
        try {
            ...
            const res = await fetch(`/api/estadistiques?${params}`, { signal: controller.signal });
    ...
    // ── Memoised derived values (avoid re-computing on every render) ──
    const mapData = useMemo(() => stats?.charts?.map || [], [stats]);
    const elevenGala = useMemo(() => stats?.rankings?.elevenGala || [], [stats]);
    ...
    return (
        <div className="min-h-screen bg-gray-50/50 flex flex-col">
            {/* Sticky filter bar */}
            <FilterPanel availableData={metaData} onFilterChange={handleFilterChange} />
            ...
            <main className="flex-1 p-4 md:p-10 space-y-12 max-w-[1600px] mx-auto w-full">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="bg-white border border-gray-200 rounded-2xl p-1 flex gap-1 shadow-sm">
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${mode === 'normal' ? 'bg-gray-900 text-white shadow' : 'text-gray-400 hover:text-gray-700'}`}
                            >
                                <BarChart3 size={13} /> Normal
                            </button>
                            <button
                                onClick={() => setMode('compare')}
                                className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${mode === 'compare' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-indigo-600'}`}
                            >
                                <GitCompare size={13} /> Compara
                            </button>
                        </div>
                        ...
                        <div className="hidden md:flex items-center gap-3 bg-white p-2 rounded-2xl border border-gray-100 shadow-sm">
                            <div className="px-3 py-2 text-right">
                                ...
                                <p className="text-xs font-bold text-gray-900 mt-1">
                                    Avui a les {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </p>
                            </div>
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                    <KpiCard title="Total Ingressos"
                        value={<PrivacyMask value={stats?.kpis?.totalIncome || 0} />}
                        subtitle="Confirmats i tancats"
                        icon={<Euro size={24} />} isLoading={loading && !stats} color="emerald" />
                    <KpiCard title="Nº de Bolos"
                        value={stats?.kpis?.count || 0}
                        subtitle={`${stats?.kpis?.confirmedCount || 0} confirmats`}
                        icon={<BarChart3 size={24} />} isLoading={loading && !stats} color="primary" />
                    <KpiCard title="Mitjana per Bolo"
                        value={<PrivacyMask value={stats?.kpis?.avgPrice || 0} />}
                        subtitle={`Mediana: ${Math.round(stats?.kpis?.medianPrice || 0)}€`}
                        icon={<TrendingUp size={24} />} isLoading={loading && !stats} color="blue" />
                    <KpiCard title="Benefici Net"
                        value={<PrivacyMask value={stats?.kpis?.netProfit || 0} />}
                        subtitle={`Despeses: ${Math.round(stats?.kpis?.totalExpenses || 0)}€`}
                        icon={<Star size={24} />} isLoading={loading && !stats} color="purple" />
                </section>

                {/* ── Loading spinner (overlay while stats update) ── */}
                <div className="relative">
                    ...
                    {!stats && loading ? (
                        ...
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="space-y-12">
                            ...
                            {mode === 'normal' && (
                                <>
                                    ...
                                    {/* Funnel */}
                                    <section className="grid grid-cols-1 gap-8">
                                        <FunnelSollicituds data={funnelData} />
                                    </section>
                                </>
                            )}

                            {/* ═══ COMPARE MODE ═══ */}
                            {mode === 'compare' && filters && (
                                <ComparePanel baseFilters={filters} availableData={metaData} />
                            )}
                        </motion.div>
                    )}
                </div>
