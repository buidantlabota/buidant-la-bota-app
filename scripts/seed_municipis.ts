...
import { normalizePlace } from "../utils/normalization.ts";
...
async function seed() {
    ...
    console.log(`🚀 Iniciant seed de ${total} municipis...`);
    ...
    for (let i = 0; i < lines.length; i += batchSize) {
        ...
        if (error) {
            console.error(`❌ Error al batch starting at ${i}:`, error.message);
        } else {
            console.log(`✅ Processats ${Math.min(i + batchSize, total)}/${total}...`);
        }
    }

    console.log("✨ Seed finalitzat corectament!");
}
